...
# for now doing these one by one until i find another way

cases_questionnaire_unnest_reached <- cases %>%
  select(id,              #get uuid for later join !
         questionnaireAnswers.communication_to_the_case
         ) %>%             #questionnaire columns
  mutate_if(is.list, simplify_all) %>%
  unnest(questionnaireAnswers.communication_to_the_case, 
         names_sep = "_") %>%
  mutate(communication_to_case = case_when(
    is.na(questionnaireAnswers.communication_to_the_case_questionnaireAnswers.communication_to_the_case) ~ questionnaireAnswers.communication_to_the_case_value,
    TRUE ~ questionnaireAnswers.communication_to_the_case_questionnaireAnswers.communication_to_the_case)) %>%
  select(id,communication_to_case)
    

cases_questionnaire_unnest_why_not_reached <- cases %>%
  select(id, questionnaireAnswers.communication_to_the_case) %>%             
  mutate_if(is.list, simplify_all) %>%
  unnest(questionnaireAnswers.communication_to_the_case, 
         names_sep = "_") %>%
  unnest(questionnaireAnswers.communication_to_the_case_subAnswers.reason,
         names_sep = "_") %>%
  rename(why_not_reached = questionnaireAnswers.communication_to_the_case_subAnswers.reason_value) %>%
  select(id,why_not_reached)


cases_questionnaire_unnest_initial_outcome <- cases %>%
  select(id, questionnaireAnswers.what_is_the_health_status_of_the_patient_at_the_initial_interview) %>%             
  mutate_if(is.list, simplify_all) %>%
  unnest(questionnaireAnswers.what_is_the_health_status_of_the_patient_at_the_initial_interview, 
         names_sep = "_") %>%
  unite(q_initial_outcome, c(2:4),
        sep = "",
        na.rm = TRUE)


cases_questionnaire_unnest_final_outcome <- cases %>%
  select(id, questionnaireAnswers.what_is_the_health_status_of_the_patient_at_the_final_interview) %>%             
  mutate_if(is.list, simplify_all) %>%
  unnest(questionnaireAnswers.what_is_the_health_status_of_the_patient_at_the_final_interview, 
         names_sep = "_") %>%
...
cases_clean <- cases %>%
  ...
  left_join(cases_questionnaire_unnest_reached, by="id") %>%   # Join back in flattened de-duped questionnaire fields above, using case id
  left_join(cases_questionnaire_unnest_why_not_reached, by="id") %>%
  left_join(cases_questionnaire_unnest_initial_outcome, by="id") %>%
  left_join(cases_questionnaire_unnest_final_outcome, by="id") 
...
contacts_becoming_cases <- cases_clean %>%
  ...
  select(
    ...
    active,
    contact_status,
    location_id, lat, long, geo_location_accurate, address, postal_code, city, telephone, email, team_id,  
    firstName, assigned_doctor_name, gender, age, age_class, occupation, pregnancy_status, 
...
contacts_clean <- contacts_clean %>%
  ...
  select(
    ...
    team_id = followUpTeamId,
    firstName,
    lastName, # be aware that when cases are merged here, the assigned doctor name will appear in last name
    gender,
    age,
    age_class,
    occupation,
