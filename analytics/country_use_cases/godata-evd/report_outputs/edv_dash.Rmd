...
total_cas_decede <- current_clean_cases %>%
  filter(outcome_id=='décédé') %>%
  count()

valueBox(total_cas_decede, 
         # icon = "fa-user-plus", 
         caption = "Décédé",
         color = "#F9D173")
...
total_cas_nondecede <- current_clean_cases %>%
  filter(outcome_id !='décédé') %>%
  count()

valueBox(total_cas_nondecede, 
         # icon = "fa-user-plus", 
         caption = "Pas décédé",
         color = "#824427")
...
df_courbe$date_of_onset <- as.Date(df_courbe$date_of_onset, "%Y-%m-%d")
...
geom_bar(stat="identity") +
  ...
  scale_x_date(date_breaks = "5 day",
               limits = as.Date(c('2021-01-13','2021-03-19', "%Y-%m-%d")))
p1
...
demograph_tab <- current_clean_cases %>%
  filter(classification=="confirmé" | classification=="probable") %>%
  filter(date_of_onset >= (today_date-1)) %>%
  select(
    full_name,
    age,
    gender,
...
demograph_tab <- current_clean_cases %>%
  filter(classification=="confirmé" | classification=="probable") %>%
  filter(date_of_onset >=  prev_7_date) %>%
  select(
    full_name,
    age,
    gender,
...
```{r donnees_demographiques-21days}

demograph_tab <- current_clean_cases %>%
  filter(classification=="confirmé" | classification=="probable") %>%
  filter(date_of_onset >=  prev_21_date) %>%
  select(
    full_name,
    age,
    gender,
...
Tous les cas {data-navmenu="LISTE DE LIGNES - INDICATEURS" data-icon=""}
===========================================================================================
