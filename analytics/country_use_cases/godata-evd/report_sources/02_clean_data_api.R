...
# %>%
# select_at(vars(id, starts_with("dateRanges"),-dateRanges_dateRanges), tolower) %>% #RDN1
...
# bind_rows(contacts_becoming_cases) #RDN1
...
cases_clean=cases_clean %>%
  replace_na(list(firstName = "", lastName = "",middleName="")) %>%
  mutate(full_name=paste0(toupper(firstName), " ",toupper(lastName), " ",toupper(middleName))) 
cases_clean=cases_clean %>%
  select(
    uuid,
    case_id,
  ...
  mutate(classification = case_when(
    ...
    classification == "NOT_A_CASE_DISCARDED" ~ "rejeté"
  )) %>%
  ...
  mutate(outcome_id = case_when(
    ...
    outcome_id == "DECEASED" ~ "décédé",
    outcome_id == "EDS" ~ "EDS",
    outcome_id == "NON_EDS" ~ "pas de EDS",
    outcome_id == "RECOVERED" ~ "rétabli"
  ))


contacts_clean=contacts_clean %>%
  replace_na(list(firstName = "", lastName = "")) %>%
  mutate(full_name=paste0(toupper(firstName), " ",toupper(lastName))) 
...
contacts_clean=contacts_clean %>%
  select(
    ...
    date_of_reporting,
    date_of_last_contact,
    followup_team_id=followUpTeamId,
    was_case,
    date_of_followup_end,
    contact_status,
    date_of_followup_start,
  ...
  mutate(gender = case_when(
    tolower(gender) == "FEMALE" ~ "femme",
    tolower(gender) == "MALE" ~ "homme"
  )) 
...
## RDN Adding scripts
###############################################################
contacts_becoming_cases_clean <- contacts_becoming_cases #adding clean at the end, to have it as a rds & csv file
# contacts_teams_clean <- left_join(contacts,teams, by=c("followUpTeamId","id"))

###############################################################
## Export Dataframes as .rds (to be ovewritten each time script is run) and as .csv (stamped with date)
###############################################################

## Specify location to save files
clean_folder_rds <- here::here("data", "clean", "rds")
clean_folder_csv <- here::here("data", "clean", "csv")
...
for (i in 1:length(mydfs)){
  # savefile<-paste0(clean_folder,"/", mydfs[i], ".csv")
  # write.csv(get(mydfs[i]), file=savefile, fileEncoding = "UTF-8", na="", row.names = F)
  
  #save files with date in filename
  savefile2<-paste0(clean_folder_csv,"/", mydfs[i], "_" , format(Sys.Date(), "%Y-%m-%d"), ".csv")
  write.csv(get(mydfs[i]), file=savefile2, fileEncoding = "UTF-8", na="", row.names = F)  
...
for (i in 1:length(mydfs)){
  savefile<-paste0(clean_folder_rds,"/", mydfs[i], ".rds")
