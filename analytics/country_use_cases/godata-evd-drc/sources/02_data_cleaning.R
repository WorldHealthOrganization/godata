...
##########################################################################
## select and Rename the column names using the country's nomenclature 
## according to the organization of the health pyramid
##########################################################################

locations_clean <- locations_clean %>% 
  mutate(pays=admin_0_name) %>% 
  mutate(province=admin_1_name) %>%
  mutate(zone=admin_2_name) %>%
  mutate(aire=admin_3_name) %>%
  mutate(quartier=admin_4_name) %>%
  select(location_id,pays,province,zone,aire,quartier) %>% 
  filter(province =="NORD-KIVU")
...
cases_hosp_history_clean <- cases %>%
  ...
  select_at(vars(id, starts_with("dateRanges")), tolower) %>%
  mutate(dateranges_typeid = sub(".*TYPE_", "", dateranges_typeid)) %>%
  mutate(dateranges_centername = sub(".*CENTRE_NAME_", "", dateranges_centername)) %>%
...
cases_clean <- cases %>%
  filter(deleted == FALSE | is.na(deleted)) %>%
  unnest(vaccinesReceived, keep_empty = TRUE) %>% #unnest vaccines because it contains a few lines with lists
...
# Delete no usable field
missing_columns_cases <- setdiff(case_columns, names(cases_clean))
cases_clean[missing_columns_cases] <- NA
# cases_clean <- cases_clean %>% 
#   select(-starts_with("followUp")) %>% 
#   select(-c(wasCase,vaccinesReceived,documents,active,addresses,dateRanges))

# capitalize the first and last name to standardize them
cases_clean <- cases_clean %>% 
...
cases_clean <- cases_clean %>%
  ...
  rename_at(vars(starts_with("dateranges")), list(~str_replace(., "dateranges", "isolation"))) %>%
  left_join(cases_hosp_history_clean %>% filter(dateranges_typeid=="HOSPITALIZATION"), by="id") %>%
  rename_at(vars(starts_with("dateranges")), list(~str_replace(., "dateranges", "hospitalization"))) %>%
  left_join(cases_hosp_history_clean %>% filter(dateranges_typeid=="VISITE_FOSA"), by="id") %>%
  rename_at(vars(starts_with("dateranges")), list(~str_replace(., "dateranges", "visite_fosa"))) %>%
  ...
  mutate(vaccine_status = sub(".*STATUS_", "", vaccine_status)) %>%
  mutate(risk_level = sub(".*LEVEL_", "", risk_level)) %>% 
  mutate(age=as.integer(age)) %>% 
  mutate(city=toupper(city)) %>% 
  left_join(select(locations_clean, quartier,aire,zone,province,location_id), by = "location_id") %>% 
  mutate(outcome_id = case_when(
    outcome_id == "ALIVE" ~ "Vivant",
    outcome_id == "DECEASED" ~ "Décedé"
  )) %>% 
  mutate(classification = case_when(
    classification  == "CONFIRMED" ~ "Confirmé",
    classification  == "PROBABLE" ~ "Probable",
    classification  == "SUSPECT" ~ "Suspect",
  )) %>% 
  mutate(case_vaccinated = case_when(
    vaccine_status  == "VACCINATED" ~ "Oui",
    vaccine_status  == "NOT_VACCINATED" ~ "Non",
    TRUE ~ "Non",
  )) %>% 
  ...
  #RDN# select importants columns cases
  select(
    uuid ,
    case_id ,
    location_id ,
...
#RDN# Add empty row
if (nrow(contacts_becoming_cases)==0) {
  contacts_becoming_cases[nrow(contacts_becoming_cases)+1,] <- NA
}
##########################################################################
...
contacts_clean <- contacts %>%
  filter(deleted == FALSE | is.na(deleted)) %>%
  unnest(vaccinesReceived,keep_empty = TRUE, names_sep = "_") %>% 
  # Remove all nested fields
  select_if(negate(is.list)) %>%
  # take out all that are not core variables
  select(-contains("questionnaireAnswers"))
...
date_fields_contacts <- c("dateOfReporting","dateOfLastContact","createdAt","updatedAt","followUp.startDate","followUp.endDate")
...
contacts_clean <- contacts_clean %>%
  ...
  select(
    ...
    occupation,
    pregnancy_status = pregnancyStatus,
    vaccine_status=vaccinesReceived_status,
    date_of_reporting,
    date_of_data_entry,
    date_of_last_contact,
    date_of_followup_start,
  ...
  #RDN#
  mutate(contact_vaccinated = case_when(
    vaccine_status  == "VACCINATED" ~ "Oui",
    vaccine_status  == "NOT_VACCINATED" ~ "Non",
    TRUE ~ "Non",
  )) %>% 
  ...
  #RDN# Select important columns to use
  select(
    uuid,
    contact_id,
    active,
...
followups_clean <- followups %>%
  filter(deleted == FALSE | is.na(deleted)) %>%
  select_if(negate(is.list)) %>%
  mutate(location_id = str_replace_na(address.locationId, replacement = "")) %>% #RDN#
  # take out all that are not core variables
  select(-contains("questionnaireAnswers")) %>%
  ...
  mutate(performed = case_when(
    statusId == "MISSED" ~ TRUE,
    statusId == "NOT_SEEN" ~ TRUE,
    statusId == "SEEN_NOT_OK" ~ TRUE,
    statusId == "SEEN_OK" ~ TRUE,
    statusId == "DECEDE" ~ TRUE,
    statusId == "REFUS_DE_SUIVI" ~ TRUE,
    statusId == "NOT_PERFORMED" ~ FALSE,
    statusId == "NOT_ATTEMPTED" ~ FALSE)) %>%
  ...
  mutate(seen = case_when(
    ...
    mutate(followup_status = case_when(
      statusId == "MISSED" ~ "missed",
      statusId == "NOT_SEEN" ~ "not_seen",
      statusId == "SEEN_NOT_OK" ~ "seen_with_signs",
      statusId == "SEEN_OK" ~ "seen_no_signs",
      statusId == "DECEDE" ~ "decede",
      statusId == "REFUS_DE_SUIVI" ~ "refus_de_suivi",
      statusId == "NOT_PERFORMED" ~ "not_performed",
      statusId == "NOT_ATTEMPTED" ~ "not_performed")) %>%
  
#RDN# Delete to converse status to prepare analysis label
  
#   mutate(followup_status = case_when(
#     statusId == "MISSED" ~ "PERDU DE VUE",
#     statusId == "NOT_SEEN" ~ "NON VU",
#     statusId == "SEEN_NOT_OK" ~ "VU AVEC SIGNE",
#     statusId == "SEEN_OK" ~ "VU SANS SIGNE",
#     statusId == "DECEDE" ~ "DECEDE",
#     statusId == "REFUS_DE_SUIVI" ~ "REFUS DE SUIVI",
#     statusId == "NOT_PERFORMED" ~ "PAS DE DONNEES",
#     statusId == "NOT_ATTEMPTED" ~ "PAS DE DONNEES")) %>% 
  
  #RDN# select important columns
  select(
    uuid ,
    contact_id,
    contact_uuid,
...
for (i in 1:length(mydfs)){
  savefile<-paste0(data_folder,"/", mydfs[i], "_",database_date,".csv")
...
for (i in 1:length(mydfs)){
  savefile<-paste0(data_folder,"/", mydfs[i],"_",database_date, ".rds")
